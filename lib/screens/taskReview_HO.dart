      ...
      body: SafeArea(
        child: SingleChildScrollView(
          child: Container(
            ...
            child: Column(
              children: [
                SPProfileDataReview(userPicture: userPicture, rating: rating, numReviews: numReviews, userName: userName,),
                Container(
                ...
                Center(
                  child: Container(
                    ...
                    child: TextButton(
                      onPressed: () async {
                        if (_providerRating > 0 && _userNotes.text.isNotEmpty) {
                         final data = await HomeOwnerReviewAPI.addOrUpdateReview(taskID, _userNotes.text,_providerRating);
                          CustomAlertDialog.showSuccessDialog(context, 'Review submitted successfully');
                          setState(() {
                            rating = data['serviceProviderRating'].toDouble();
                            numReviews = data['serviceProviderNumReviews'];
                          });

                        }
                        else
                          {
                            CustomAlertDialog.showErrorDialog(context, 'Please provide a rating and review notes');
                          }
                      },
                      child: const Text(
                        'Save',
